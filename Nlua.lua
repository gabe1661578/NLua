...
function Nlua:new(layers, learningRate, threshold)
...
function Nlua:decode(hiddenSignal)
...
function Nlua:bp(inputs, outputs)
...
function Nlua:saveNetwork(network, savefile)
	print(savefile)
	persistence.store(savefile, network)
end

function Nlua:loadNetwork(savefile)
	local ann = persistence.load(savefile)
		ann.bp = Nlua.bp
		ann.activate = Nlua.activate
		for i = 1, #ann do
...
function Nlua:loadTrainingDataFromFile(fileName)
local trainingData = {}
local fileLines = {}
